...
def find_ocr_files(search_dir):
    return sorted(search_dir.rglob("batch_*_ocr.csv"))
...
def main(gold_csv_path, ocr_search_dir, out_path):
    ...
    for (menu_id, page_id), group in grouped:
        ocr_part = ocr_df[(ocr_df["menu_id"] == menu_id) & (ocr_df["page_id"] == page_id)].copy()
        ...
        for _, row in ocr_part.iterrows():
            ...
            ocr_lines.append(
                {
                    "text": line_text,
                    "line_order": int(row.get("line_order")) if not pd.isna(row.get("line_order")) else None,
                    "ocr_confidence": float(row.get("ocr_confidence")) if not pd.isna(row.get("ocr_confidence")) else None,
                    "bbox_x1": float(row.get("bbox_x1")) if not pd.isna(row.get("bbox_x1")) else None,
                    "bbox_y1": float(row.get("bbox_y1")) if not pd.isna(row.get("bbox_y1")) else None,
                    "bbox_x2": float(row.get("bbox_x2")) if not pd.isna(row.get("bbox_x2")) else None,
                    "bbox_y2": float(row.get("bbox_y2")) if not pd.isna(row.get("bbox_y2")) else None,
                }
            )
    ...
    stats = {
        ...
        "layout_fields": ["text", "line_order", "ocr_confidence", "bbox_x1", "bbox_y1", "bbox_x2", "bbox_y2"],
    }
    stats_path = out_path.parent / "build_full_gold_layout_stats.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gold_csv_path", type=str, default="data/processed/gold/menu_gold_all_batches.csv")
    parser.add_argument("--ocr_search_dir", type=str, default="data/processed/annotation_batches")
    parser.add_argument("--out_path", type=str, default="data/processed/gold/menu_gold_all_batches_layout.jsonl")
